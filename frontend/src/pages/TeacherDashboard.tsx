...
export default function TeacherDashboard() {
    const { socket, activePoll, pollResults, participants, history } = useSocket('teacher', 'Teacher');
    ...
    useEffect(() => {
        if (viewingHistory && socket) {
            socket.emit('request_history');
        }
    }, [viewingHistory, socket]);
    ...
    return (
        <div style={{ width: '100%', maxWidth: '1000px', display: 'flex', flexDirection: 'column', gap: '20px', padding: '40px' }}>
            {/* Top Bar */}
            <div style={{ display: 'flex', justifyContent: 'flex-end', width: '100%' }}>
                <button
                    onClick={() => setViewingHistory(!viewingHistory)}
                    style={{ backgroundColor: 'var(--primary)', color: 'white', border: 'none', borderRadius: '24px', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 600 }}
                >
                    {viewingHistory ? 'Back to Dashboard' : (
                    ...
                    {viewingHistory ? (
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                {history.length === 0 ? <p>No polls have been conducted yet.</p> : history.map((poll, pIdx) => (
                                    <div key={pIdx}>
                                        <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '16px' }}>Question {history.length - pIdx}</h2>
                                        <div className="card" style={{ overflow: 'hidden' }}>
                                            <div style={{ padding: '16px 24px', backgroundColor: 'var(--text-dark)', color: 'white', fontWeight: 600 }}>
                                                {poll.question}
                                            </div>
                                            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                                {poll.options.map((opt: any, idx: number) => {
                                                    let percentage = 0;
                                                    const resultsArr = poll.results;
                                                    const totalVotes = resultsArr.reduce((acc: number, val: any) => acc + val.count, 0);
                                                    const optVotes = resultsArr.find((r: any) => r.optionIdx === idx)?.count || 0;
                                                    percentage = totalVotes === 0 ? 0 : Math.round((optVotes / totalVotes) * 100);

                                                    return (
                                                        <div key={idx} style={{
                                                            position: 'relative', padding: '12px 20px', borderRadius: '8px', border: '1px solid #E5E5E5', overflow: 'hidden', display: 'flex', justifyContent: 'space-between'
                                                        }}>
                                                            <div style={{
                                                                position: 'absolute', left: 0, top: 0, bottom: 0, width: `${percentage}%`, backgroundColor: 'var(--primary-light)', zIndex: 0
                                                            }} />
                                ...
                                <div>
                                    ...
                                    {timeLeft === 0 && (
                                        <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end' }}>
                                            <button className="btn" style={{ padding: '14px 40px', borderRadius: '24px' }} onClick={() => window.location.reload()}>+ Ask a new question</button>
                                        </div>
                                    )}
                                </div>
                            )}
